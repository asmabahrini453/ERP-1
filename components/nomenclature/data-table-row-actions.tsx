...
import { nomenclatureFormSchema } from "@/lib/validations/schema";
...
export function DataTableRowActions<TData>({ row, onEdit, onDelete }: DataTableRowActionsProps<TData>) {
  ...
  const nomenclature = nomenclatureFormSchema.parse(row.original);
    ...
    setDialogContent(
      ...
      nomenclatureData={nomenclature}
        onSave={(updatedNomenclature) => {
          onEdit(updatedNomenclature); // Call parent handler
  ...
  const handleDelete = () => {
    onDelete(nomenclature);
    toast({
      title: "Success",
      description: "Nomenclature supprimée avec succès!",
    });
