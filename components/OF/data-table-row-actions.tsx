...
import { nomenclatureFormSchema, ofFormSchema } from "@/lib/validations/schema";
...
export function DataTableRowActions<TData>({ row, onEdit, onDelete }: DataTableRowActionsProps<TData>) {
  ...
  const of = ofFormSchema.parse(row.original);
  ...
  const handleEditClick = () => {
    setDialogContent(
      ...
      ofData={of}
        onSave={(updatedOf) => {
          onEdit(updatedOf); // Call parent handler
  ...
  const handleDelete = () => {
    ...
    toast({
      title: "Success",
      description: "Ordre de fabrication supprimée avec succès!",
    });
