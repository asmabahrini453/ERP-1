...
import { workstationData } from "@/app/data";
import { DataTable } from "@/components/data-table";
import LoadingScreen from "@/components/Loading";
import { Button } from "@/components/ui/button";
import { columns, Workstation } from "@/components/workstation/columns";
import CreateWorkstationSheet from "@/components/workstation/modals/create-workstation";
...
const WorkstationList = () => {
  ...
  const [workstations, setWorkstations] = useState<Workstation[]>(workstationData);
  const [sheetOpen, setSheetOpen] = useState(false);

  const handleEdit = (updated: Workstation) => {
    setWorkstations((prev) =>
      prev.map((workstation) => (workstation.id === updated.id ? updated : workstation))
    );
  };

  const handleDelete = (workstationToDelete: Workstation) => {
    setWorkstations((prev) =>
      prev.filter((worksation) => worksation.id !== workstationToDelete.id)
    );
  };

  const handleCreate = (newWorkstation: Workstation) => {
        ...
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <h1 className="text-2xl text-[#383861] font-bold">
                Liste des sations des travails
              </h1>
              <p className="text-muted-foreground mt-1">
                Gérez vos sations des travails et leurs détails
              </p>
            </div>
            ...
            <div className="flex items-center md:gap-4 sm:gap-1 sm:mt-1">
              ...
              <div className="flex md:gap-4 sm:gap-1 sm:mr-2">
                ...
                >
                  ...
                  <span className="hidden md:inline">Nouvelle sation de travail</span>
                </Button>
              </div>
            </div>
          </div>
          <div className="bg-white rounded-lg shadow-sm p-4">
            <DataTable
              ...
              filterKey="nom"
              filterPlaceholder="Filtrer par Nom de station..."
              columnLabels={{
                nom: "Nom",
                capacite: "Capacité de travail",
                heures: "Total des Heures Travaillées",
                type: "Type",
                statut: "Statut",
              }}
            />
